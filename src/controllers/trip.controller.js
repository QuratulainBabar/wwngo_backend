...
export const acceptSenderRequest = asyncHandler(async (req, res) => {
  const requestService = await import('../services/trip_sender_request.service.js');
...
export const declineSenderRequest = asyncHandler(async (req, res) => {
  const requestService = await import('../services/trip_sender_request.service.js');
  const data = await requestService.declineSenderRequest(
    req.user.id,
    req.params.requestId
  );
  res.json({ success: true, data });
...
export const acceptSenderCounterOffer = asyncHandler(async (req, res) => {
  const offerService = await import('../services/trip_counter_offer.service.js');
  const paymentMethod =
    req.body?.paymentMethod === 'wallet' ? 'wallet' : 'stripe';
