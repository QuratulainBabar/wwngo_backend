...
export async function ensureConversation({
  ...
  const existing = await pool.query(
    `SELECT * FROM conversations WHERE delivery_id = $1`,
    [deliveryId]
  );
  if (existing.rows[0]) return existing.rows[0];

  const { rows } = await pool.query(
    `INSERT INTO conversations (delivery_id, participant_a_id, participant_b_id)
     VALUES ($1, $2, $3)
     ON CONFLICT (delivery_id) DO UPDATE SET updated_at = NOW()
     RETURNING *`,
    [deliveryId, participantAId, participantBId]
  );
  return rows[0];
}
